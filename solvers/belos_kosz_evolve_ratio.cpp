...
struct resultcmp{
  result res1;
  result res2;
  bool operator>(const resultcmp& rhs) const {
    return res1.flops*1.0/res2.flops > rhs.res1.flops*1.0/rhs.res2.flops;
  }
};
...
result solvekosz(int argc, char* argv[], LLA* mat) {
  ...
  tie(newx, flag, relres, iter, resvec, work) = S.solve(b, 1e-12, maxiters);
  clock_t t_end = clock();
  h.freeMemory();
  g.freeMemory();
  kosz.freeMemory();
...
int main(int argc, char *argv[]) {
  std::priority_queue<resultcmp, std::vector<resultcmp>, std::greater<resultcmp> > q;
  namespace stdfs = std::experimental::filesystem;
  stdfs::path path=argv[1];
  std::string outpath=argv[2];
  std::string method1=argv[3];
  std::string method2=argv[4];
  std::ofstream resultinfo;
  resultinfo.open(argv[5],std::ios::app);
  ...
  for(int i=0; i < filenames.size(); ++i) {
    ...
    try{
      result trial1=solve(argc-6,argv+6,matrix,method1);
      result trial2=solve(argc-6,argv+6,matrix,method2);
      
      if(trial1.success == 0 && trial2.success == 0) {

	std::string outfile=outpath+"/"+stdfs::path(filenames[i]).filename().string();
	trial1.filename=outfile;
	trial2.filename=outfile;
	resultcmp compare;
	compare.res1=trial1;
	compare.res2=trial2;
	if(q.size() < 100) {
	  stdfs::copy(filenames[i],outfile);
	  q.push(compare);
	}
	else if(compare > q.top()){
	  resultcmp temp=q.top();
	  q.pop();
	  remove(temp.res1.filename.c_str());
	  q.push(compare);
	  stdfs::copy(filenames[i],outfile);
	}
  ...
  if(atoi(argv[6])==0) {
    ...
    std::cerr << temp.res1.filename << std::endl;
    std::cerr << method1 << "     iters: " << temp.res1.iters << " work: " << temp.res1.flops << " residual: " << temp.res1.residual << std::endl;
    std::cerr << method2 << "     iters: " << temp.res2.iters << " work: " << temp.res2.flops << " residual: " << temp.res2.residual << std::endl << std::endl;

    resultinfo << temp.res1.flops << " " << temp.res2.flops << std::endl;
    resultinfo.close();
    return 0;
  }
  ...
  for(int i=0; i < filenames.size(); ++i) {
    ...
    for(int j=i+1; j < filenames.size(); ++j) {
      ...
      LLA* matrix2 = MMA_read(filenames[j]);
	...
	return 1;
      ...
      int mutatect=atoi(argv[6]);
      std::uniform_int_distribution<> dis2(1,(int)matrix->n/10);
      int maxexchange=(int)matrix->n/10;
      
      LLA_mod mutate(matrix,maxexchange+abs(off1));
      
      for(int i=0; i < mutatect; ++i) {
	int exchange=dis2(gen);
	if(off1 > 0) {
	  mutate.remove_edges(exchange+off1);
	  mutate.add_edges(exchange,1,1000);
	}
	else if (off1 < 0){
	...
	if(matrix->m != fixedm) {
	  std::cerr << "complain 1 " << matrix->m << std::endl;
	  ...
	  if(connected(matrix)) {
	    try{
	      ...
	      if(trial1.success == 0 && trial2.success == 0) {
		std::string outfile=outpath+"/"+random_string(32)+".mtx";
		while(exists(outfile)) {
		  outfile=outpath+"/"+random_string(32)+".mtx";
		}
		trial1.filename=outfile;
		trial2.filename=outfile;
		resultcmp compare;
	...
	mutate.replace_edges();
      ...
      LLA_mod mutate2(matrix2,maxexchange+abs(off2));
      for(int i=0; i < mutatect; ++i) {
	int exchange=dis2(gen);
	if(off2 > 0) {
	  mutate2.remove_edges(exchange+off2);
	  mutate2.add_edges(exchange,1,1000);
	}
	else if (off2 < 0){
	...
	if(matrix2->m != fixedm) {
	  std::cerr << "complain 3 " << matrix2->m << std::endl;
	}
	else {
	  ...
	  if(connected(matrix2)) {
	    try{
	      result trial1=solve(argc-6,argv+6,matrix2,method1);
	      result trial2=solve(argc-6,argv+6,matrix2,method2);
