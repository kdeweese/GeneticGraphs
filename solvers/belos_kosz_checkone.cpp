...
result solvekosz(int argc, char* argv[], LLA* mat) {
  ...
  if(flag==0) {
    ...
    std::cerr << flag << " " << relres << " " << iter << std::endl;
    trial.success=-1;
    return trial;
  }
...
int main(int argc, char *argv[]) {
  ...
  LLA* matrix = MMA_read(filename);
  try{
    result trial1=solve(argc-2,argv+2,matrix,method1);
    
    if(trial1.success == 0) {
      std::cerr << trial1.flops << std::endl;
    }
    else {
      std::cerr << "fail with code " << trial1.success << std::endl;
    }
  }
  catch(...){}
  matrix->freeMemory();
