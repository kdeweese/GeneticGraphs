...
int main(int argc, char *argv[]) {
  ...
  resultinfo.open(argv[4],std::ios::app);
  double minweight=1;
  ...
  for(int i=0; i < filenames.size(); ++i) {
    ...
    try{
      result trial1=solve(argc-5,argv+5,matrix,method1);
  ...
  if(atoi(argv[5])==0) {
    ...
    std::cerr << temp.filename << std::endl;
    std::cerr << method1 << "     iters: " << temp.iters << " work: " << temp.flops << " residual: " << temp.residual << std::endl;
    
    resultinfo << temp.flops  << std::endl;
    resultinfo.close();
    return 0;
  }
  int basedebug=q.top().flops;
  std::cerr << "progress(";
  for(int i=0; i < filenames.size(); ++i) {
	...
	if(off1 > 0) {
	  ...
	  mutate.add_edges(exchange,minweight,maxweight);
	}
	else if (off1 < 0){
	  mutate.remove_edges(exchange);
	  mutate.add_edges(exchange-off1,minweight,maxweight);
	}
	else {
	  mutate.remove_edges(exchange);
	  mutate.add_edges(exchange,minweight,maxweight);
	...
	if(matrix->m != fixedm) {
	  ...
	  if(connected(matrix)) {
		...
		if(q.top().flops < basedebug) {
		  std::cerr << "complain " <<std::endl;
		  std::cerr << trial1.filename << " " << trial1.flops << std::endl;
		}
	      }
	    }
	    catch(...){}
	...
	if(off2 > 0) {
	  ...
	  mutate2.add_edges(exchange,minweight,maxweight);
	}
	else if (off2 < 0){
	  mutate2.remove_edges(exchange);
	  mutate2.add_edges(exchange-off2,minweight,maxweight);
	}
	else {
	  mutate2.remove_edges(exchange);
	  mutate2.add_edges(exchange,minweight,maxweight);
	...
	if(matrix2->m != fixedm) {
	  ...
	  if(connected(matrix2)) {
	    try{
	      result trial1=solve(argc-5,argv+5,matrix2,method1);
